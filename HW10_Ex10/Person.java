...
public class Person {
	...
	protected Person(String firstName, String middleName, String lastName, char gender, LocalDate dateOfBirth,
		...
		this.currentAddress = new Address(country, city, municipality, postalCode, street, number, floor, apartamentNo);
	...
	public String toString() {
		...
		try {
			if (this.gender == 'M') {
				...
				String secondLine = String.format("He " + this.currentAddress.toString() + "He started %s degree in %s on "
						+ this.ed.get(lastIndex).getEnrollmentData() + " and ", this.getDegree(), this.ed.get(lastIndex).getInstutionName());
				firstLine = firstLine + secondLine;
				if (this.ed.get(lastIndex).isGraduated()) {
					String lastLine = String.format(
							"finished on " + this.ed.get(lastIndex).getGraduationDate() + ". His grade was %.3f.",
							((GradedEducation) this.ed.get(lastIndex)).getFinalGrade());
					firstLine = firstLine + lastLine;

				} else {
					String lastLine = String.format("is supposed to graduate on " + this.ed.get(lastIndex).getGraduationDate() + ".");
					firstLine = firstLine + lastLine;
				}
				return firstLine;
			}
			...
			String secondLine = String.format("She " + this.currentAddress.toString() + "She started %s degree in %s on "
					+ this.ed.get(lastIndex).getEnrollmentData() + " and ", this.getDegree(), this.ed.get(lastIndex).getInstutionName());
			firstLine = firstLine + secondLine;
			if (this.ed.get(lastIndex).isGraduated()) {
				String lastLine = String.format("finished on " + this.ed.get(lastIndex).getGraduationDate() + ". Her grade was %.3f.",
						((GradedEducation) this.ed.get(lastIndex)).getFinalGrade());
				firstLine = firstLine + lastLine;
			} else {
				String lastLine = String.format("is supposed to graduate on " + this.ed.get(lastIndex).getGraduationDate() + ".");
